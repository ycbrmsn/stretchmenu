 ...
 *   1.1.3 2018-02-08 加上菜单伸缩时间配置参数stretchMenuSpeed
 *   1.1.4 2018-03-07 加上菜单的边框，配上红色的样式
 */
;(function (factory) {
  ...
  $.fn.stretchmenu = function (options) {
    var defaultOption = {
      ...
      stretchMenuSpeed: 300, // 收缩/伸展菜单时间(单位毫秒)
  ...
  function getListStr(data, level, opt) {
    ...
    sb += '<div class="stretchmenu-slidebox">'
        ...
        + '</div>'
    // 菜单边框
    sb += '<div class="stretchmenu-menuline__top"></div>';
    sb += '<div class="stretchmenu-menuline__bottom"></div>';
    sb += '<div class="stretchmenu-menuline__left"></div>';
    sb += '<div class="stretchmenu-menuline__right"></div>';
    sb += '</div>';
    return sb;
  }
  ...
  function toggleMenu($root, opt) {
    ...
    $menuItemsShow.animate({
      'width': menuWidth + 'px'
    }, opt.stretchMenuSpeed, 'swing', function () {
      if (opt.isMenuStretched) {
        // 显示更多图标
        $moreIcons.show();
      }
